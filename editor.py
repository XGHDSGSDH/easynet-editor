...
class editor:
    def __init__(self, x_pos, y_pos, tkwindow):
        ...
        self.rander_target = Canvas(self.window, width=200 + self.window.winfo_width(),
                                    height=200 + self.window.winfo_height(), background="green")
        ...
        # tensor按钮
        self.btn_tensor = Button(self.window, activeforeground="black", text="add_tensor", font=("Arial Bold", 16),
                                 bg="blue", fg="black")
        self.btn_tensor.grid(row=0, column=0)
        self.btn_tensor.bind("<Button-1>", self.add_tensor)

        # layer列表
        self.layer_combobox = tkinter.ttk.Combobox(self.window, width=35, height=30, values=["dense", "relu", "add",
                                                                                             "choose what type of layer you want to add"])
        self.layer_combobox.current(3)
        self.layer_combobox.grid(row=0, column=1)

        # layer按钮
        self.btn_layer = Button(self.window, activeforeground="black", text="add_layer", font=("Arial Bold", 16),
                                bg="blue", fg="black")
    ...
    def add_layer(self, event):
        ...
        self.vector_layer.append(layer(self.window.winfo_x() + self.window.winfo_width() / 2,
                                       ...
                                       self.layer_combobox.get(), int(self.question_front.get()),
                                       int(self.question_end.get()), self.layer_num, self))
