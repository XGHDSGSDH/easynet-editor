...
class layer:
    def __init__(self,x,y,name,num,neweditor):
        ...
        self.arrow1=0
        self.arrow2=0
        self.fill_color = "#F9906F"
        self.text_color = "#FFFFFF"
        self.qwget()
    def qwget(self):
        self.qw=layer_qw(self)
    def draw_my_self(self):#重新画自己的圆和字和箭头
        self.circle=self.new_editor.rander_target.create_oval(self.cmpx+self.new_editor.center_x-self.radius,self.cmpy+self.new_editor.center_y-self.radius,self.new_editor.center_x+self.cmpx+self.radius,self.cmpy+self.new_editor.center_y+self.radius,fill=self.fill_color,width=0)
        self.text=self.new_editor.rander_target.create_text(self.cmpx+self.new_editor.center_x,self.cmpy+self.new_editor.center_y,text=self.type,font=("Consolas",16),fill=self.text_color)
...
class dense(layer):
    ...
    def printo(self):
        string1="dense "+str(self.front)+" "+str(self.end)+" "+str(self.inlen)+" "+str(self.outlen)+"\n"
